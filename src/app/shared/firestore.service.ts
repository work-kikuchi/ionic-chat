...
export class FirestoreService {
  ...
  constructor(
    ...
    this.messageCollection = this.fireStore.collection<IMessage>('chat', ref=>ref.orderBy('timestamp','desc'));
  ...
  chatInit():Observable<IChat[]>{
    return this.messageCollection.valueChanges({idField:'messageId'}).pipe(
      concatMap(async message=>{
        const users = await firstValueFrom(this.userCollection.valueChanges({idField:'uid'}).pipe(
          first()));
